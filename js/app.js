...
current={region,cuisine,dish,time:now()};ui.renderList("historyList",st.addHistory(current),"尚無紀錄");ui.setFavorite(st.favorites().some(x=>x.dish===dish));ui.reveal();window.HFSound.success();ui.setHunting(false);setTimeout(()=>ui.setStage(-1),600)}
...
(async()=>{refresh();const r=await fetch("./data/foods.json");data=await r.json();w.render(Object.keys(data));if("serviceWorker"in navigator)addEventListener("load",()=>navigator.serviceWorker.register("./sw.js"))})()})();
